...
//added by chen
void clflush_array(unsigned char *sm, unsigned long long size) {
  for (unsigned long long i = 0; i < size; i += 8){ // cahce line of x86 cache is 64bytes
    _mm_clflush((unsigned char *)sm + i);
  }

  _mm_mfence();
}

void clflush_warray(poly *arr, unsigned int size){
  for(unsigned int i; i < size; i += 64){
    _mm_clflush((poly *)arr + i);
  }
  _mm_mfence();
}
               ...
               const polyveck *w1)
{
  ...
  for(i = 196; i < 256; ++i) {
    do {
      ...
      b = outbuf[pos++];
     clflush_array(outbuf, SHAKE256_RATE);
    } while(b > i);
                ...
                const unsigned char *sk)
{
  ...
  //add by chen
  //if(cacheflush(sm, mlen, BCACHE) != 0){
  //	printf("error: no cache flush\n");
  //}
  //asm("wbinvd");
  //clflush_array(sm, mlen + CRYPTO_BYTES);
  //printf("%d\n", CRYPTO_BYTES);
  //tmpArray = flush_array();
  /* Compute CRH(tr, msg) */
